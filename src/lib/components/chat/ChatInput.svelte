<script lang="ts">
	...
	import IconGoogleG from "../icons/IconGoogleG.svelte"; // Ensure IconGoogleG is imported
	import { useSettingsStore } from "$lib/stores/settings";
	import { webSearchParameters } from "$lib/stores/webSearchParameters";
	import Switch from "../Switch.svelte"; // Ensure Switch is imported
	...
	const onFileChange = async (e: Event) => {
		// Prevent adding files if Google Search is on or loading
		if ($webSearchParameters.googleSearchIsOn || loading) return; 
		if (!e.target) return;
	...
	function handleKeydown(event: KeyboardEvent) {
		if (event.key === "Enter" && !event.shiftKey && !isCompositionOn && !isVirtualKeyboard() && value.trim() !== "") {
	...
	let webSearchIsOn = $derived(
		modelHasTools
			? ($settings.tools?.includes(webSearchToolId) ?? false) || ($settings.tools?.includes(fetchUrlToolId) ?? false)
	...
	// Explicitly type as ToolFront[] and remove 'satisfies'
	let extraTools: ToolFront[] = $derived( 
		page.data.tools
			.filter((t: ToolFront) => $settings.tools?.includes(t._id))
			.filter((t: ToolFront) => ![documentParserToolId, imageGenToolId, webSearchToolId, fetchUrlToolId].includes(t._id))
	);

	// Visibility logic
	let showWebSearch = $derived(!assistant && env.PUBLIC_ENABLE_WEB_SEARCH_TOGGLE === 'true'); 
	let showGoogleSearch = $derived(!assistant && env.PUBLIC_ENABLE_GOOGLE_SEARCH_TOGGLE === 'true'); 
	...
	let showNoTools = $derived(!showWebSearch && !showGoogleSearch && !showImageGen && !showFileUpload && !showExtraTools);
</script>

<div class="flex min-h-full flex-1 flex-col" onpaste={onPaste}>
	<textarea rows="1" tabindex="0" inputmode="text" class="scrollbar-custom max-h-[4lh] w-full resize-none overflow-y-auto overflow-x-hidden border-0 bg-transparent px-2.5 py-2.5 outline-none focus:ring-0 focus-visible:ring-0 max-sm:text-[16px] sm:px-3" class:text-gray-400={disabled} bind:value bind:this={textareaElement} onkeydown={handleKeydown} oncompositionstart={() => (isCompositionOn = true)} oncompositionend={() => (isCompositionOn = false)} oninput={adjustTextareaHeight} onbeforeinput={(ev) => { if (page.data.loginRequired) { ev.preventDefault(); $loginModalOpen = true; } }} {placeholder} {disabled}></textarea>

	{#if !showNoTools}
		<div class="scrollbar-custom -ml-0.5 flex max-w-[calc(100%-40px)] flex-wrap items-center justify-start gap-2.5 px-3 pb-2.5 pt-1.5 text-gray-500 dark:text-gray-400 max-md:flex-nowrap max-md:overflow-x-auto sm:gap-2">
			{#if showWebSearch}
				<HoverTooltip label="Search the web" position="top" TooltipClassNames="text-xs !text-left !w-auto whitespace-nowrap !py-1 !mb-0 max-sm:hidden {webSearchIsOn ? 'hidden' : ''}">
					<button class="base-tool" class:active-tool={webSearchIsOn} disabled={loading} onclick={async (e) => { e.preventDefault(); if (modelHasTools) { if (webSearchIsOn) { await settings.instantSet({ tools: ($settings.tools ?? []).filter((t) => t !== webSearchToolId && t !== fetchUrlToolId) }); } else { await settings.instantSet({ tools: [...($settings.tools ?? []), webSearchToolId, fetchUrlToolId] }); } } else { webSearchParameters.update(params => ({...params, useSearch: !params.useSearch})); } }}>
						<IconInternet classNames="text-xl" />
						{#if webSearchIsOn} Search {/if}
					</button>
				</HoverTooltip>
			{/if}
			{#if showGoogleSearch}
			...
			{/if}
			{#if showFileUpload}
				{@const isFileUploadDisabled = loading || $webSearchParameters.googleSearchIsOn}
				{@const mimeTypesString = mimeTypes.map((m) => m.endsWith("*") ? m.split("/")[0] : m.split("/")[1]).join(", ")}
				<div class="flex items-center">
					<HoverTooltip label={`${mimeTypesString.includes("*") ? "Upload any file" : `Upload ${mimeTypesString} files`}${ isFileUploadDisabled ? ' (disabled while Google Search is on or loading)' : ''}`} position="top" TooltipClassNames="text-xs !text-left !w-auto whitespace-nowrap !py-1 !mb-0 max-sm:hidden">
						<label class="base-tool relative" class:active-tool={documentParserIsOn} class:opacity-50={isFileUploadDisabled} class:cursor-not-allowed={isFileUploadDisabled}>
							<input disabled={isFileUploadDisabled} class="absolute hidden size-0" aria-label="Upload file" type="file" onchange={onFileChange} accept={mimeTypes.join(",")} />
							<IconPaperclip classNames="text-xl" />
							{#if documentParserIsOn} Document Parser {/if}
						</label>
					</HoverTooltip>
				</div>
				{#if mimeTypes.includes("image/*")}
					<HoverTooltip label={`${"Capture screenshot"}${ isFileUploadDisabled ? ' (disabled while Google Search is on or loading)' : ''}`} position="top" TooltipClassNames="text-xs !text-left !w-auto whitespace-nowrap !py-1 !mb-0 max-sm:hidden">
						<button class="base-tool" class:opacity-50={isFileUploadDisabled} class:cursor-not-allowed={isFileUploadDisabled} disabled={isFileUploadDisabled} onclick={async (e) => { e.preventDefault(); if (isFileUploadDisabled) return; const screenshot = await captureScreen(); const base64Response = await fetch(screenshot); const blob = await base64Response.blob(); const file = new File([blob], "screenshot.png", { type: "image/png" }); files = [...files, file]; }}>
...
<style lang="postcss">
	:global(pre), :global(textarea) { font-family: inherit; box-sizing: border-box; line-height: 1.5; }
</style>
